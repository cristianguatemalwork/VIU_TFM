...
La normativa de la Seguridad Social en Ecuador está principalmente regulada por la \estcite{ConsEcu:2008}, específicamente en el artículo 34, donde se establece lo siguiente:
...
Para acceder a estas prestaciones, los individuos deben cumplir determinados requisitos establecidos en la \estcite{LeySS}. Una vez cumplidos, el monto de las prestaciones es determinado según lo dispuesto en la \estcite{ResIESS_CD100}, específicamente en el artículo 13. Este documento establece que la pensión mensual por invalidez o vejez, así como el subsidio transitorio por incapacidad, se calcularán multiplicando la base de cálculo, obtenida de acuerdo al procedimiento descrito en el artículo 2 de la misma resolución, por el coeficiente anual de años cumplidos de imposiciones \citep{iessivm2020}. Donde, la base de cálculo se determina como el salario promedio de los cinco mejores años de la vida laboral del individuo.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{graficos/kmeans.png}
...
Según \textcite{kanagala2016comparative}, DBScan es un algoritmo de agrupamiento basado en la densidad, ya identifica las regiones de alta concentración de puntos, las cuales están separadas entre sí por áreas de baja concentración. En este algoritmo, los puntos se clasifican en tres tipos: punto central, aquellos que tienen más del mínimo de puntos especificado por el usuario (MinPts) dentro de un radio específico que, de igual forma, lo determina el usuario (Eps); punto de frontera, los puntos que dentro de un radio Eps tienen menos puntos que el número MinPts, pero un punto central se encuentra en su vecindad; y puntos de ruido, que son aquellos que no son centrales ni de frontera
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{graficos/dbscan.png}
\caption{\headlinecolor{\underline{Ejemplo de implementación del algoritmo DBSCAN}}}
\label{fig:dbscan}
\vspace{2mm}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{graficos/jerarquico.png}
...
Este algoritmo usa el método de Bagging para obtener múltiples conjuntos de entrenamiento que sean diferentes entre ellos. Cada uno de estos conjuntos genera un árbol de decisión en el cual se usan atributos aleatorios. Debido a la gran cantidad de árboles que este algoritmo genera, se concluye que el mismo es bueno para resistir el ruido y tiene una buena capacidad de clasificación. Dependiendo de la información dada, cada árbol generará un resultado y, finalmente, con la integración de los resultados de todos los árboles se obtendrá el resultado final \citep{moreno2023manual}.
...
Para el balanceo de clases se usan los métodos oversampling y undersampling. De donde, el oversampling se encarga de generar copias de ejemplos de la clase minoritaria, pero puede incrementar el riesgo de sobreajuste. Mientras que, el undersampling elimina ejemplos de la clase mayoritaria para equilibrar el conjunto, pero puede descartar datos útiles, afectando negativamente la capacidad del modelo para generalizar.

Las principales técnicas usadas para llevar acabo los distintos tipos de balanceo son SMOTE (Synthetic Minority Over-sampling Technique) y SMOTE + ENN (Edited Nearest Neighbors).

\begin{itemize}
\item SMOTE es un método de oversampling que genera nuevos ejemplos sintéticos de la clase minoritaria interpolando con las instancias existentes, lo que reduce el sobreajuste en modelos de clasificación. Este logra que las fronteras de decisión de la clase minoritaria vayam más adentro del espacio de la clase mayoritaria, lo que mejora el rendimiento del modelo.
...
\item Leave-One-Out Cross-Validation (LOOCV): Es un tipo de la técnica k-Fold donde k es el número total de observaciones. Cada observación se deja fuera una vez, y el modelo es entrenado con el resto de observaciones. Una desventaja del mismo es que puede ser computacionalmente costoso.
\end{enumerate}

\subsubsection{Método del Codo}\label{sec:codo}

Según \textcite{yuan2019kvalue}, el método del codo es una técnica bastante usada para determinar el número de clústers en los algoritmos de agrupamiento, sobre todo en K-means. Su propósito es encontrar el número de clústers, donde si se aumentan más, ya no hay una mejora significativa en la calidad de las agrupaciones. Este método, en primer lugar, grafica la suma se lo errores cuadrados dentro de los clústers dependiendo el número de clústers elegidos, k. Con el aumento de k, este error disminuye, pero después de cierto punto, la mejora se ve reducica y forma una curva semejante a un codo. Donde, el codo marca el número óptimo de clústers ya que a partir de ellos el resultado no se ve significativamente mejorado.

\subsubsection{Análisis de componentes principales (ACP)}\label{sec:acp}
...
Según \textcite{FAWCETT2006861}, una de las herramientas más comunes para evaluar el rendimiento de un modelo de clasificación, la matriz de confusión. La misma tiene como objetivo medir el funcionamiento de un algoritmo comparando los valores reales con los resultados obtenidos con el mismo, donde los resultados pueden formar parte de cuatro categorías:

\begin{itemize}
\item Verdaderos positivos (VP): Casos donde el modelo predice una clase positiva incorrectamente.
...
\item Falsos Negativos (FN): Casos donde se prdice una clase negativa de forma incorrecta.
\end{itemize}
...
\begin{itemize}
\item Sensibilidad: Mide cúan bien un modelo identifica las instancias positivas, es decir, mide cuántas observaciones positivas se identificaron correctamente.
...
Una vez entrenado el modelo se ha obtenido que la exactitud del mismo al ser aplicado a la base de datos de entrenamiento es de __\%. Para contrastar este resultado y evaluar el modelo se obtiene la matriz de confusión de las predicciones realizadas de la base de datos de testeo, que se encuentra a continuación:
...
\cellcolor[HTML]{FFA074} \textbf{Real}
 & \textbf{No atípico}  & a: (TN) & b: (FP) \\ \cline{2-4}
\cellcolor[HTML]{FFA074} & \textbf{Atípico} & c: (FN) & d: (TP) \\ \hline
\end{tabular}
\end{table}

A partir de estos resultados, podemos extraer ciertas métricas que nos ayudarán a establecer la capacidad de predicción del modelo, es decir, si su funcionamiento es óptimo o no dependiendo de nuestros objetivos. Para ello, calcularemos las siguientes métricas:
...
En primer lugar, se puede observar que la exactitud del modelo probado en los datos de prueba y en los datos de testeo difiere en ___ puntos porcentuales. De donde, al ser una diferencia mínima podemos afirmar que el modelo no presenta sobreajuste. Específicamente, tenemos que el porcentaje de predicciones correctas, exactitud, del modelo aplicado a los datos de testeo es de __\%, lo que nos permite deducir que el modelo está siendo un buen clasificador debido a que existe un porcentaje alto de datos clasificados correctamente. Por otro lado, el número de datos atípicos clasificados correctamente, la sensibilidad, es de __\%. Es por ello que tenemos valores óptimos en ambas métricas, debido a que se tiene un porcentaje aceptable de valores clasificados correctamente. Y de igual forma, observamos que el número de atípicos que no fueron detectados es bajo.

% General
En este estudio, las medidas clave para la evaluación del modelo serán tanto la sensibilidad como la exactitud. Esto ya que la exactitud proporciona una visión general de qué tan bien el modelo clasifica los datos globalmente. No obstante, es necesario tener en cuenta los casos en los que los valores atípicos se han clasificado incorrectamente como no atípicos, lo que se obtiene restando a 1 la sensibilidad. Dado el contexto del estudio, la detección de valores atípicos es crucial, ya que la falta de identificación de uno de ellos puede generar más problemas que la mala clasificación de un valor no atípico como atípico. En este estudio, no detectar un atípico representa no identificar a una persona que ha cometido fraude, lo cual puede resultar en grandes pérdidas para la Seguridad Social. Por el contrario, si una persona que no ha cometido fraude es identificada como un valor atípico, únicamente se realizará una investigación más profunda, sin que esto resulte en mayores pérdidas económicas.



Random Forest

Este algoritmo construye múltiple árboles de decisión, es un modelo de ensable ya que mejora su predicción al promediar los resultados de varios árboles de decisión individuales. Cada árbol es construido con muestras aleatorias lo que ayuda a reducir la varianza y el sobreajuste que afecta a los árboles de decisión individuales. Además, entre otras ventajas encontramos que este algoritmo es eficiente en cuanto a las variables, ya que da métricas a la importancia de las variables y manerja la multicolinealidad de las mismas. También, maneja mejor los datos desbalanceados. Finalmente, dado a su flexibilidades con la detección de interacciones más complejas su capacidad predictiva es mayor. he2018random

Bagging

El Bootstrap Aggregating también conocido como Bagging es un modelo de ensable que mejora la precisión y estabilidad de modelos al reducir la varianza a través de la creación de subconjuntos aleatorios doonde se entrenan modelos independientes y cuyo resultado final se determina por voto mayoritario entre cada submodelo. Entre sus ventajas vemos que por uso de Bootstrap la varianza disminuye, se evita el sobreajuste y mejora la predicción. De igual forma, por su manera de implementación permite entramiento en paralelo que reduce el tiempo necesario. Finalemnte, vemos que su desempeño es bueno incluso cuando hay datos faltantes o hay una gran cantidad de variables.
evolutionary_bagging_2022


KNN

Este es un método no paramétrico que clasifica los puntos según su distancia a los datos de entrenamiento. Se basa es que los puntos cercanos en el espacio de características suelen pertenecer a la misma clase, por lo que a los nuevos puntos se les asigna la etiqueta más común entre sys vecinos. Entre sus ventajas encontramos su facilidad de implementación, su flesibilidad cuando no se conoce la estructura subyacente del modelo, eficacia especialmente cuando hay una buena distribución de datos, y su robustez al ruido ya que permite ajustes para evitar influencia de valores atípicos. Finalmente, su rendimiento depende de la elección del parámetro k y la distancia usada.
syriopoulos2023knn

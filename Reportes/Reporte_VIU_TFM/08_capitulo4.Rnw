...
Con esto se entrena el modelo y tenemos que el accuracy del train y test es 0.9264488334801364 y 0.8252817489893173, respectivamente. Para contrastar este resultado y evaluar el modelo se obtiene la matriz de confusión que se encuentra en la tabla \ref{tab:XGBClassifier_random_search} y en la figura \ref{fig:confusion_matrix_algoritmo_XGBClassifier_random_search} en anexos.
...
\label{tab:XGBClassifier_random_search}
\end{table}

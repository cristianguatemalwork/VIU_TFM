...
El primer paso es la generación de un adecuado proceso KKD (en función de lo descrito en la sección xxxxx), para que a partir de los datos primarios que dispone el IESS, generar el conocimiento adecuado que permita dar solución al problema planteado. En las siguientes secciones se muestra cada una de las fases del proceso KKD.
...
componente se encuentra cargado dentro del esquema pensiones\_owner}. Adicionalmente, la información se complementa con los datos proporcionados por el Registro Civil (entidad pública que guarda los datos personales de la población ecuatoriana).
...
\begin{multicols}{2}  
    ...
    \begin{itemize}  
        ...
        \item PROMEDIO\_SUELDO\_REAL: Corresponde a la base de cálculo\footnote{Para el cómputo de la base de cálculo de la pensión, se procederá a la suma de doce (12) meses de imposiciones consecutivas y ese resultado se dividirá para doce (12). Obteniendo así el promedio mensual de los sueldos o salarios de cada año de imposiciones del afiliado, se seleccionarán los cinco (5) promedio mensuales de mayor cuantía y el resultado de la suma se dividirá para cinco (5)} del promedio de los años de mejores sueldos o salarios sobre los cuales se aportó dentro del sistema Prestacional,\\[-0.75cm]
        \item PROMEDIO\_CAL: Corresponde a la base de cálculo estimada en función de los cinco (5) mejores años de sueldo sobre los cuales aportó,\\[-0.75cm]
...
En este sentido, en la primera fila de la figura \ref{fig:algoritmo_cj_depu} se muestra la clasificación del algoritmos, y en la segunda, las correcciones realizadas en base a lo expuesto con anterioridad. 
...
Los métodos que nos servirán para estimar el número de clústeres, para los datos de \textit{ul} y \textit{ml}, son por el método del codo y por el método de la silueta.
...
Tomando esto en cuenta, una vez entrenado el modelo con los parámetros previamente adquiridos, se ha obtenido que el acurracy del mismo al ser aplicado en los datos del test es de 0,7483230571176909. Para contrastar este resultado y evaluar el modelo se obtiene la matriz de confusión de las predicciones realizadas de la base de datos de testeo, que se encuentra en la tabla \ref{tab:mc_knn_1} y en la figura \ref{fig:matriz_conf_knn_acp} en anexos.
...
A partir de estos resultados, podemos extraer ciertas métricas, a parte del accuracy que nos ayudarán a establecer la capacidad de predicción del modelo, es decir, si su funcionamiento es óptimo o no dependiendo de nuestros objetivos. Para ello, tenemos que en este modelo la sensibilidad es de 0.48, la especifidad es de  y finalmente la exactitud es de .
...
Una vez entrenado el modelo con estos parámetros, se ha obtenido que el acurracy del mismo al ser aplicado en los datos del test es de 0.7128644668789665. Para contrastar este resultado y evaluar el modelo se obtiene la matriz de confusión que se encuentra en la tabla \ref{tab:mc_knn_2} y e n la figura \ref{fig:matriz_conf_knn_bal_acp} en anexos.
...
A partir de estos resultados obtenemos que este modelo tiene sensibilidad de 0.28, especifidad es de  y finalmente exactitud  de .
...
Considerando a \textit{variables\_ml}, se implementa un único árbol de deción realizando la validación cruzada. Los parámetros generales que se consideran son que el númeto de fold es 10, \textit{gini} como  criterio de evaluación de las divisiones, la profundidad del árbol estára en el rango de 2 a 100 y la medida de éxisto será el accuracy.
...
Una vez que el modelo ha sido entrenado con estos parámetros, se ha obtenido que el acurracy del mismo al ser aplicado en los datos del train y test son de 0.8432114867764302 y 0.7994218217133049 respectivamente. Para contrastar este resultado y evaluar el modelo se obtiene la matriz de confusión que se encuentra en la tabla \ref{tab:mc_knn_2} y en la figura \ref{fig:confusion_matrix_algoritmo_arbol_simple} en anexos.
...
\cellcolor[HTML]{FFA074} \textbf{Real}   & \textbf{No atípico}  & a: (TN) & b: (FP) \\ \cline{2-4} 
\cellcolor[HTML]{FFA074}                  & \textbf{Atípico}     & c: (FN) & d: (TP) \\ \hline
\end{tabular}
}
\caption{\headlinecolor{\underline{Matriz de confusión para el KNN con balanceo y \textit{variables\_ml}}}}
\label{tab:mc_knn_2}
\end{table}

A partir de estos resultados obtenemos que este modelo tiene sensibilidad de 0.21, especifidad es de  y finalmente exactitud  de .

La cuatro variablaes más importantes son \textit{BASE\_CAL} (28,07\%), \textit{LS2} (19,46\%), \textit{LS\_MS} (16,21\%) y \textit{M\_PRI\_P} (14,52\%)

Se repite el mismo proceso, pero realizando un balanceo de clases, con lo cual tenemos que en la figura \ref{fig:algortimo_arbol_simple_bal}, la profunidad del árbol es de 15.
...
La cuatro variablaes más importantes son \textit{BASE\_CAL} (33,23\%), \textit{LS2} (18,98\%), \textit{N\_PRI\_P	} (18,21\%) y \textit{SAL\_PROM2	} (6,98\%).

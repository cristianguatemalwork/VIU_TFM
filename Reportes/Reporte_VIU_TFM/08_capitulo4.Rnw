...
El resultado de aplicar el Test de Esfericidad da un p-valor menor a 0, lo cual permite concluir que estadísticamente las variables están correlacionadas. No obstante, el valor de KMO es 0,4720, lo cual da señales que no es factible\footnote{En \citet{tesis_guatemal_epn} se manifiesta que si los valores del KMO están entre 0,5 y 1 es apropiado aplicar un ACP} realizar un ACP. En este sentido, se tienen los suficientes argumentos para no realizar el ACP. No obstante, con la finalidad de comparar los resultados que nos darán los métodos supervisados y el ACP (solo se considerará a las componentes del ACP para el caso del algoritmo de KNN) se procede a implementarlo.
...
Se sigue el mismo proceso descrito en la subsección anterior, por lo que, para el rango de valores que se presenta en la subsección \ref{sec_anexos:conjunto_modelos_AdaBoostClassifier} de anexos y utilizando RandomizedSearchCV, se tiene que los parámetros son:

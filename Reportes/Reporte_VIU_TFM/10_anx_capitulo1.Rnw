...
El repositorio en Github tiene por nombre \textit{VIU\_TFM}, en el mismo se encuentra la carpeta \textit{SQL} con los script .sql utilizados en la construcción de la data \textit{data\_afi}, la carpeta \textit{Python} que contiene los scripts .ipynb con los códigos utilizados en todo el proceso KDD, implementación de los algoritmos de clúster jerárquico, K-Means y DBSCAN, así como los códigos de las técnicas de aprendizaje supervisado de KNN, árboles de decisión, conjunto de modelos, XGBoost y las redes neuronales y la carpeta \textit{R} contiene los scrip .R utilizados en la edición del presente proyecto en formato \texttt{\LaTeX}. En enlace de acceso es haciendo \href{https://github.com/cristianguatemalwork/VIU_TFM}{click aqui}

...
De esta forma, el principal problema que este trabajo de titulación busca abordar es la detección de aportaciones fraudulentas que realizaron los actuales pensionstas del Seguro IVM durante su vida laboral (con el objetivo de recibir una pensión de mayor cuantía) y generar un modelo de aprendizaje supervisado para poder clasificar el comportamiento salarial de los afiliados en curso. 

Para lograr este fin, las técnicas de aprendizaje no supervisado de clusterización jerárquica, k-mean y dbscan, permitirán identificar  si dentro de las cotizaciones históricas realizadas por los pensionistas, existen aportes fraudulentos (atípicos). Posteriormente, mediante una adecuada selección de características (dadas por el análisis de componentens principales y dbscan), validación cruzada y optimización de hiperparámetros, se implementará el algoritmo KNN y las técnicas de árboles de decisión, conjuntos de modelos, XGBClassifier y redes neuronales para generar modelos de aprendizaje supervisado que permitan predecir como se comportarán los sueldos los actuales afiliados. Todos estos modelos serán comparados con la métrica del accuracy.

En el segundo capítulo se detalla el objetivo general y los específicos  alcanzar con el presente trabajo de fin de máster.
...
En el cuarto capítulo se detalla todo el proceso de minería de datos (dado por el proceso KDD), selección de característícas (mediante el análisis de componentes principales, cluster jerarqucio y dbscan) y los diferentes resultados alcanzados tras la implementación de los diferentes algoritmos y técnicas de aprendizaje supervizado y no supervisado. Así como un analísis comparativo de los mismos.

En el apartado final se describen las conclusiones del trabajo de titulación, asi como las limitaciones y futuros pasos a realizar en relación al problema abordado.
